...
func TestCreateMemoComment_ReturnsParentRelation(t *testing.T) {
	...
	relation := comment.Relations[0]
	assert.Equal(t, v1pb.MemoRelation_COMMENT, relation.Type)
	require.NotNil(t, relation.Memo)
	require.NotNil(t, relation.RelatedMemo)
	assert.Equal(t, comment.Name, relation.Memo.Name)
	assert.Equal(t, parent.Name, relation.RelatedMemo.Name)
}
