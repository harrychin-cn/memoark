...
func (d *DB) ImportMemosAtomically(ctx context.Context, creatorID int32, memos []*store.Memo) error {
	...
	if err := tx.QueryRowContext(ctx, `SELECT id FROM "user" WHERE id = $1 FOR UPDATE`, creatorID).Scan(&lockedCreatorID); err != nil {
		return errors.Wrap(err, "failed to lock memo import target")
	}
	var exists bool
	if err := tx.QueryRowContext(ctx, "SELECT EXISTS(SELECT 1 FROM memo WHERE creator_id = $1 LIMIT 1)", creatorID).Scan(&exists); err != nil {
	...
	const statement = "INSERT INTO memo (uid, creator_id, created_ts, updated_ts, row_status, content, visibility, pinned, payload) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)"
